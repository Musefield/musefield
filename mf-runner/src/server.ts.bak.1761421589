...
// --- Static UI ---
// Serve the compiled app at /ui/, and (belt-and-suspenders) expose /assets as well.
app.use("/ui", express.static("/workspace/app/dist", { index: "index.html" }));
app.use("/assets", express.static("/workspace/app/dist/assets"));
app.get("/ui", (_req, res) => res.sendFile("/workspace/app/dist/index.html"));
